...
summ = tf.summary.merge([summ_Dloss, summ_Dloss])
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epoch):
        ...
        for genome_filename in glob.glob(h5_files):
            ...
            da_ta = h5f.root.data
            data_size = len(da_ta)
            print (genome_filename, len(da_ta))
            # Train
            ...
            for idx in perm:
                ...
                if global_step % 1000 == 1:
                    if global_step == 1:
                        saver.save(sess, 'gfl_model', global_step=global_step)
                    # Validation
                    ...
                    saver.save(sess, 'gfl_model', global_step=global_step, write_meta_graph=False)
